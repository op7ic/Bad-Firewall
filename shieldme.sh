...
echo [+] Downloading IPs for tor exit node blocklist from https://www.dan.me.uk/torlist/
curl -s -A "Firefox" https://www.dan.me.uk/torlist/ | sort | uniq > tor_current_nodes_torlist.txt 2> /dev/null

echo [+] Downloading IPs for tor exit node blocklist from http://www.talosintelligence.com/documents/ip-blacklist
...
echo [+] Downloading IPs for bruteforce hosts blocklist from https://jamesbrine.com.au/csv
curl -s -A "Firefox" https://jamesbrine.com.au/csv | awk -F "," '{print $1}' | sort | uniq | grep -v ipv4 > bruteforce-ips.txt 2> /dev/null

echo [+] Downloading IPs for abuse.ch urls and extra IPs blocklist from https://urlhaus.abuse.ch/downloads/text/
...
echo [+] Downloading IPs for abuse.ch blocklist from https://feodotracker.abuse.ch/downloads/ipblocklist.txt
...
echo [+] Downloading IPs for threatview.io Twitter List blocklist from https://threatview.io/Downloads/Experimental-IOC-Tweets.txt
...
ipset create threatview_c2feed hash:net hashsize 32768 maxelem 9999999
while read line; do ipset add threatview_c2feed $line; done < threatview_c2feed_ips.txt 2>/dev/null
...
ipset create threatview_twitterfeed hash:net hashsize 32768 maxelem 9999999
while read line; do ipset add threatview_twitterfeed $line; done < threatview_twitterfeed_ips.txt 2>/dev/null
...
ipset create abusechtracker1 hash:net hashsize 32768 maxelem 9999999
while read line; do ipset add abusechtracker1 $line; done < urlhausabuse-ips.txt 2>/dev/null
...
ipset create abusechtracker2 hash:net hashsize 32768 maxelem 9999999
while read line; do ipset add abusechtracker2 $line; done < urlhausabuse-ips2.txt 2>/dev/null
...
while read line; do ipset add tor-individual-ip1 $line; done < tor_current_nodes.txt 2>/dev/null
...
while read line; do ipset add tor-individual-ip2 $line; done < tor_current_nodes_torlist.txt 2>/dev/null
...
while read line; do ipset add alienvault $line; done < alienvault.txt 2>/dev/null
...
while read line; do ipset add ssh $line; done < ssh.txt 2>/dev/null
...
ipset create bruteforce-ips hash:net hashsize 32768 maxelem 16777216
while read line; do ipset add bruteforce-ips $line; done < bruteforce-ips.txt 2>/dev/null
...
while read line; do ipset add Talos $line; done < Talos.txt 2>/dev/null
...
while read line; do ipset add ipv6 $line; done < ipv6.txt 2>/dev/null
...
for z in "${!array[@]}"
  do
   ...
   while read line; do ipset add $z $line; done < $z.txt 2>/dev/null
